    ...
    <md-table v-model="searched" md-sort="name" md-sort-order="asc" md-card md-fixed-header>
      <md-table-toolbar>
        <div class="md-toolbar-section-start">
          <h1 class="md-title">DICOM 标签值</h1>
        </div>
      ...
      <md-table-row slot="md-table-row" slot-scope="{ item }">
        <md-table-cell md-label="标签名" md-sort-by="name" md-numeric>{{ item.name }}</md-table-cell>
        <md-table-cell md-label="标签值" md-sort-by="value">{{ item.value }}</md-table-cell>
      </md-table-row>
    </md-table>
  </div>
</template>
